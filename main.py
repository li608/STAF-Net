...
def get_parser():
    ...
    parser.add_argument(
        '--config',
        default='./config/nturgbd-cross-subject/test_joint.yaml',
...
class Processor():
    ...
    def __init__(self, arg):
        ...
        if arg.phase == 'train':
            if not arg.train_feeder_args['debug']:
                ...
                self.train_writer = SummaryWriter(os.path.join(arg.model_saved_name, 'debug_train'), 'debug_train')
                self.val_writer = SummaryWriter(os.path.join(arg.model_saved_name, 'debug_test'), 'debug_test')
        self.global_step = 0
        self.load_model()
        self.load_optimizer()
        self.load_data()
    ...
    def load_model(self):
        ...
        self.loss = LabelSmoothing(smoothing=0.01).cuda(output_device)
    ...
    def feature_extract(self, epoch, loader_name=['test']):
        self.model.eval()
        self.print_log('Eval epoch: {}'.format(epoch + 1))
        for ln in loader_name:
            all_feature = None
            ...
            for batch_idx, (data, label, index) in enumerate(process):
                with torch.no_grad():
                    ...
                    #     label.long().cuda(self.output_device),
                    #     requires_grad=False)
                    feature = self.model.module.feature_extract(data)
                    if all_feature is None:
                        all_feature = feature.cpu().numpy()
                    else:
                        all_feature = np.concatenate((all_feature,feature.cpu().numpy()),axis=0)
                    # if batch_idx >=3:
                    #     break

            np.save(self.arg.work_dir+'/features_{}.npy'.format(ln),all_feature)
    ...
    def start(self):
        ...
        elif self.arg.phase == 'feature':
            if self.arg.weights is None:
                raise ValueError('Please appoint --weights.')
            self.arg.print_log = False
            self.print_log('Model:   {}.'.format(self.arg.model))
            self.print_log('Weights: {}.'.format(self.arg.weights))
            self.feature_extract(epoch=0, loader_name=['test'])
            self.print_log('Done.\n')
