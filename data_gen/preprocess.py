...
if __name__ == '__main__':
    # 定义需要处理的文件路径列表
    input_files = [
        '../data/ntu120/xsetup/val_data_joint.npy',
        '../data/ntu120/xsetup/train_data_joint.npy',
        '../data/ntu120/xsub/val_data_joint.npy',
        '../data/ntu120/xsub/train_data_joint.npy'
    ]

    output_files = [
        '../data/ntu120/xsetup/val_data_joint_pre.npy',
        '../data/ntu120/xsetup/train_data_joint_pre.npy',
        '../data/ntu120/xsub/val_data_joint_pre.npy',
        '../data/ntu120/xsub/train_data_joint_pre.npy'
    ]

    # 确保输入文件和输出文件数量匹配
    assert len(input_files) == len(output_files), "输入文件和输出文件数量不匹配"

    # 遍历每个文件，加载、处理并保存
    for input_file, output_file in zip(input_files, output_files):
        print(f'Processing file: {input_file}')
        data = np.load(input_file)
        processed_data = pre_normalization(data)
