...
def read_xyz(file, max_body=2, num_joint=25):
    ...
    return data
#
# # 你可以根据关节索引表定义有效的17个关节连接。假设这些关节索引为
# valid_joints = [0, 1, 4, 5, 6, 7, 9, 10, 11, 13, 14, 15, 16, 17, 18, 19, 20]  # 假设有效关节
#
# # 根据17个关节重新定义关节连接
# arms = [11, 10, 9, 8, 7, 6]  # 假设的手臂连接
# rightHand = [11, 24]  # 右手
# leftHand = [7, 22]  # 左手
# legs = [19, 18, 17, 16, 15, 14, 13]  # 腿部
# body = [0, 1, 20]  # 其他身体部分
## 2D展示
def Print2D(num_frame, point, arms, rightHand, leftHand, legs, body):
    ...
    for i in range(n, m):
        ...
        plt.axis('off')  # 关闭坐标轴（坐标线 + 坐标值）

        # 画出两个body所有关节
        plt.scatter(point[0, i, :, 0], point[1, i, :, 0], c='red', s=40.0)  # c: 颜色;  s: 大小
        ...
        # 在关节点位置显示编号
        for j in range(point.shape[2]):  # 遍历每个关节点
            plt.text(point[0, i, j, 0], point[1, i, j, 0], str(j), fontsize=10, color='blue')  # 显示编号
            plt.text(point[0, i, j, 1], point[1, i, j, 1], str(j), fontsize=10, color='blue')  # 显示编号
...
def Print3D(num_frame, point, arms, rightHand, leftHand, legs, body):
    ...
    for i in range(n, m):
        ...
        # 画出两个body所有关节
        plot3D.scatter(point[0, i, :, 0] * Expan_Multiple, point[1, i, :, 0] * Expan_Multiple, point[2, i, :, 0],
        ...
        plot3D.set_xlim3d(xmin - 0.7, xmax + 0.7)  # x坐标范围
        plot3D.set_ylim3d(ymin - 0.3, ymax + 0.3)  # y坐标范围
        plot3D.set_zlim3d(zmin - 0.7, zmax + 0.7)  # z坐标范围
        plt.savefig('skeleton.svg', format='svg')
        plt.pause(0.001)  # 停顿延时

        # 在第6帧时保存为SVG文件
        if i == 6:
            # 设置保存路径
            save_path = './skeleton_images/frame6.svg'  # 设置保存路径为当前目录下的 "skeleton_images" 文件夹
...
def main():
    ...
    data_path = '../data/nturgbd_raw/nturgb+d_skeletons/S010C001P019R001A010.skeleton'  # 拍手skeleton文件名
    ...
    #Print2D(num_frame-60, point, arms, rightHand, leftHand, legs, body)  # 2D可视化
    Print3D(num_frame-60, point, arms, rightHand, leftHand, legs, body) # 3D可视化
